...
var self$2 = commonjsGlobal || /* istanbul ignore next */ {};
...
var cjs$1 = self$2.CustomEvent;

/*! (c) Andrea Giammarchi - ISC */

var self$3 = commonjsGlobal || /* istanbul ignore next */ {};
...
const reraf$1 = (m => m.__esModule ? /* istanbul ignore next */ m.default : /* istanbul ignore next */ m)(cjs$4);
const umap = (m => m.__esModule ? /* istanbul ignore next */ m.default : /* istanbul ignore next */ m)(cjs$5);
...
const useReducer = (reducer, value, init, options) => {
  ...
  if (i === length) {
    ...
    ref._ = asy ? (updates.get(hook) || updates.set(hook, reraf$1())) : hookdate;
    ref.f = value => {
...
var dropEffect_1 = dropEffect;

const hasEffect = effects.has.bind(effects);
var hasEffect_1 = hasEffect;

const useEffect = createEffect(true);
var useEffect_1 = useEffect;
...
const observe = cjs$3({Event: cjs$1, WeakSet: cjs$2});
...
const augmentor$1 = fn => {
  let handler = null;
  const hook = augmentor_1(fn);
  return function () {
    const node = hook.apply(this, arguments);
    if (hasEffect_1(hook))
      observer(node, handler || (handler = dropEffect_1.bind(null, hook)));
    return node;
  };
};
...
var VOID_ELEMENTS_1 = VOID_ELEMENTS;
...
var cjs$7 = {
	...
	VOID_ELEMENTS: VOID_ELEMENTS_1
};

var cjs$8 = createCommonjsModule(function (module, exports) {
/*! (c) Andrea Giammarchi - ISC */

const {UID, UIDC, VOID_ELEMENTS} = cjs$7;
...
const persistent = fragment => {
  ...
  const nodes = slice$1.call(childNodes, 0);
 ...
 */
var cjs$a = (parentNode, a, b, get, before) => {
...
const trim$1 = (m => m.__esModule ? /* istanbul ignore next */ m.default : /* istanbul ignore next */ m)(cjs$c);

const {
  UID: UID$1, UIDC: UIDC$1, UID_IE: UID_IE$1, COMMENT_NODE: COMMENT_NODE$1, ELEMENT_NODE: ELEMENT_NODE$2, SHOULD_USE_TEXT_CONTENT: SHOULD_USE_TEXT_CONTENT$1, TEXT_NODE: TEXT_NODE$1
} = cjs$7;
...
/* istanbul ignore next */
var normalizeAttributes = UID_IE$1 ?
  ...
  while (i < length) {
    ...
    switch (child.nodeType) {
      ...
      case COMMENT_NODE$1:
        ...
        if (textContent === UID$1) {
          parts.shift();
          holes.push(
            // basicHTML or other non standard engines
            // might end up having comments in nodes
            // where they shouldn't, hence this check.
            SHOULD_USE_TEXT_CONTENT$1.test(node.nodeName) ?
              Text(node, path) :
      ...
      case TEXT_NODE$1:
        ...
        if (
          SHOULD_USE_TEXT_CONTENT$1.test(node.nodeName) &&
          trim$1.call(child.textContent) === UIDC$1
        ) {
          parts.shift();
...
function parseAttributes(node, holes, parts, path) {
  ...
  while (i < length) {
    ...
    var direct = attribute.value === UID$1;
    var sparse;
    if (direct || 1 < (sparse = attribute.value.split(UIDC$1)).length) {
  ...
  /* istanbul ignore next */
  var cleanValue = 0 < length && UID_IE$1 && !('ownerSVGElement' in node);
  while (i < length) {
...
// globals
const WeakMap = (m => m.__esModule ? /* istanbul ignore next */ m.default : /* istanbul ignore next */ m)(cjs);

// utils
const createContent = (m => m.__esModule ? /* istanbul ignore next */ m.default : /* istanbul ignore next */ m)(cjs$9);
const importNode = (m => m.__esModule ? /* istanbul ignore next */ m.default : /* istanbul ignore next */ m)(cjs$b);
const trim = (m => m.__esModule ? /* istanbul ignore next */ m.default : /* istanbul ignore next */ m)(cjs$c);
const sanitize = (m => m.__esModule ? /* istanbul ignore next */ m.default : /* istanbul ignore next */ m)(cjs$8);
const umap = (m => m.__esModule ? /* istanbul ignore next */ m.default : /* istanbul ignore next */ m)(cjs$5);

// local
const {find, parse} = walker;
...
var domtagger = /*@__PURE__*/getDefaultExportFromCjs(cjs$d);
...
var cjs$e = hyperStyle;

const {isArray: isArray$1} = cjs$6;
  ...
  attribute(node, name, original) {
    ...
    switch (name) {
      case 'class':
        ...
        name = 'className';
      case 'props':
        return setter_1(node, name);
      case 'aria':
        return aria_1(node);
      case 'style':
        return cjs$e(node, original, isSVG);
      case 'ref':
        return ref_1(node);
      case '.dataset':
        return data_1(node);
      ...
      if (oldValue !== value) {
        ...
        if (type === 'object' && value) {
          if ('text' in value) {
            ...
            textContent(slice_1.call(value).join(''));
          }
...
const unroll = (Tagger, info, {type, template, values}) => {
  ...
  if (!entry || (entry.template !== template || entry.type !== type)) {
    ...
    info.entry = (entry = {
      ...
      template,
      tag,
      wire: persistent_1(tag(template, ...values))
    });
  }
  else
...
const cache$1 = cjs$5(new cjs);
const render$1 = (where, what) => {
  const hook = typeof what === 'function' ? what() : what;
  const info = cache$1.get(where) || cache$1.set(where, createCache$1(null));
  info.w = where;
  info.W = what;
  return render(
    where,
...
const createHook = (info, entry) => augmentor$1(function () {
...
const createCache$1 = p => ({p, stack: [], entry: null});

const unroll$1 = (info, {fn, template, values}) => {
  let {entry} = info;
...
const unrollValues$1 = (info, values, length) => {
  const {stack} = info;
  for (let i = 0; i < length; i++) {
    const hook = values[i];
    if (hook instanceof Hook)
      values[i] = unroll$1(stack[i] || (stack[i] = createCache$1(info)), hook);
    else if (hook instanceof Hole)
      unrollHole(stack[i] || (stack[i] = createCache$1(info)), hook);
    else if (isArray_1(hook))
      unrollValues$1(stack[i] || (stack[i] = createCache$1(info)), hook, hook.length);
    else
      stack[i] = null;
  }
  if (length < stack.length)
...
function createFor(lighter) {
  ...
  return (
    (entry, id) => {
      const store = cache.get(entry) || cache.set(entry, create$1(null));
      const info = store[id] || (store[id] = createCache$1(null));
      return (
        (template, ...values) => {
...
const PinMarkRow = Component((url, child) => html$1 `
...
const NowPointer = Component((now) => {
    let pointerEl = null;
    useEffect_1(() => {
        pointerEl?.scrollIntoView({ inline: 'center' });
    }, []);
    return html$1 `
        <div id=pointer ref=${(el) => pointerEl = el} style=${{
...
const Timeline = Component((now) => html$1 `
    <div id=timeline>
        ${NowPointer(now)}
        ${new Array(24).fill(0).map((v, i) => html$1 `
...
const Schedule = Component((now, channels) => html$1 `
    <div id=schedule>
        ${Timeline(now)}
        <div id=channels style=${{ maxHeight: `calc(var(--base-size) * ${channels.length})` }}>
            ${channels.map(ch => PinMarkRow(ch.images.logo, ch.schedules.map(sc => html$1 `
...
const isNow = (now, { start, end }) => now > start && now < end;
const calcWidth = (sc) => `calc(var(--day-width) / ${dayMins$1 / getLength(sc)})`;
const getLength = (sc) => getMins(sc.end) - getMins(sc.start);
const dayMins$1 = 60 * 24;
const getMins = (d) => d.getMinutes() + d.getHours() * 60;

const EPG = Component((channels) => {
    const now = new Date();
    return html$1 `
        ${Days(now)}
 ...
 */
const makeRequest = (baseUrl, baseOptions = {}) => (url, options, status) => request(baseUrl + url, lodash.merge({}, baseOptions, options), status);
/**
...
const request$1 = makeRequest('http://localhost:1337');
const getEPG = () => request$1('/epg').then(r => r.json()).then((r) => {
    for (const ch of r.channels)
        for (const schedule of ch.schedules) {
...
const Body = Component(() => {
    const [{ loading, epg, error }, setState] = useState_1({
        loading: getEPG(),
        epg: null,
        error: null,
    });
    useEffect_1(() => {
        if (!loading)
            return;
        loading.then(epg => setState({ loading: null, epg, error }), error => setState({ loading: null, epg, error }));
    }, []);
    return html$1 `
    ${Header()}
    <main>
